...
1. **After backend commits**: Check the commit message
   - If it says "API change", "new endpoint", "update DTO" → Update markdown
   - If it says "bug fix", "internal refactor" → May not need update

2. **Quarterly review**: Re-verify against latest Swagger JSON
...
- ❌ Bug fixes that don't affect API contract

### Update Checklist

- [ ] Read commit message to understand change
- [ ] Compare against Swagger JSON for accuracy
- [ ] Update commit tracking table
- [ ] Add/modify affected endpoints
- [ ] Add/modify affected DTOs
- [ ] Add/modify affected enums
